...
if(dataset$calc_commutes[1] == "Yes"){
  ...
  acm_commutes$Commute.Rank[is.na(acm_commutes$Commute.Time)] <- NA
...
# library(RCurl)
# 
# acm_commutes <- read.csv(file = paste(root_dir, "ACM Commutes.csv", sep = ""), check.names=FALSE, stringsAsFactors = FALSE)
# 
# acm_commutes$Commute.Time <- as.numeric(as.character(acm_commutes$Commute.Time))
#   
# acm_commutes <- acm_commutes %>% 

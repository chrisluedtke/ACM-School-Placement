# ACM Placement Algorithem
# v0.9
# Developed by Alex Perusse and Chris Luedtke
#
# The purpose of this version is to have a single script which can be run in its entireity in approximately
# the same way that the Power BI version will. 

# Set the package dependancies for this script
dependencies <- c("gmapsdistance",
                  "readxl",
                  "googleway",
                  "dplyr",
                  "tidyr",
...
#install.packages(dependencies, quiet = T)
lapply(dependencies, require, character.only = TRUE)

# load data
# We'll need to get the path from the Power BI parameter
acm_df <- read.csv(file = "Input 1 - ACM Data.csv") %>%
            mutate(acm_id = 1:nrow(.))
...
school_df <- read_excel(path = "Input 3 - School Data.xls") %>%
               mutate(sch_id=1:nrow(.))

# Helper Functions (To be called in other functions)
...
# We aim for a uniform distribution when it makes sense.
school_config <- function(school_df, acm_enc){
  ...
  school.data <- select(school_df, `sch_id`, `Team Size`, `School Type`, `% Caucasian`:`% N/A`, `N Spanish Speakers Reqd`) %>%
...
hs_loss <- function(targets, actuals){
  loss <- ifelse(targets == 0, (targets - actuals)^2 * 1e10, (targets - actuals)^2)
  sum(loss)
}

reqd_spanish_loss <- function(targets, actuals, min_reqd = F){
  loss <- if (min_reqd == T) {
    ifelse(actuals < targets, (targets - actuals)^2 * 1e10, (targets - actuals)^2)
  } else {
    (targets - actuals)^2
  }
  sum(loss)
...
calculate_score = function(team_placements_df, school_targets, gender_target=gender_g) {
  ...
  scores$Spanish <- reqd_spanish_loss(placed$SpanishNeed, placed$Spanish, min_reqd = F)
  #scores$Spanish <- reqd_spanish_loss(school_targets$`N Spanish Speakers Reqd`, placed$Spanish, min_reqd = T)
...
# Final Product
placement <- function(acm_df, school_df, acm_commutes){
...
output <- placement(acm_df, school_df, acm_commutes)

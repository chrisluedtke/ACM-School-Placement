...
# Rename survey items to variable names
survey_df <- read.csv(file = "survey_sample.csv")
vars_df <- read_excel(path = "Survey Items to Variable Names.xls")

for (x in names(survey_df)[2:ncol(survey_df)]){
  names(survey_df)[names(survey_df) == x] <- vars_df$`Variable Name`[vars_df$`Survey Item` == x]
}

...
## To Do
  * Determine weights
  * Set parameter inputs (i.e. consider_roommates = FALSE)
  * Set firm restrictions on random placements (make random swap -> check if valid placement)
      - no roommates same team (if roommate parameter = TRUE)
...
## Chris
  ...
  * Age variance Score
    * remaining challenge: add weights and probably remove some aspects from the loop
  * ethnicity
    * difficult since multi-ethnic
  * attend CY school - seems like this will mostly result in a manual placement on a case-by-case basis
...
generate_fake_df <- function(num_rows, school_df, seed = 42){
  ...
  fake_df <- data.frame(
    ...
    Manual.Placement = sample( c(NA, school_df$`School Name`), num_rows, replace=TRUE, prob=c(0.98, rep(0.001, length(school_df$`School Name`))))
  )
...
encode_acm_df <- function(df){
  ...
  # Ed Attainment
  acm_enc$Ed_HS <- grepl("High School/GED", df$Educational.Attainment) + grepl("Some College", df$Educational.Attainment)
  acm_enc$Ed_Col <- grepl("Bachelor's Degree", df$Educational.Attainment) + grepl("Associate's Degree", df$Educational.Attainment) + grepl("Master's Degree", df$Educational.Attainment)
...
corps_demographic_targets <- function(school_df, acm_enc){
  ...
  distros$education <- group_by(acm_enc, Ed_HS) %>% 
    summarise(count=n()) %>% 
...
school_config <- function(school_df, acm_enc){
  ...
  school.data <- select(school_df, `School_ID`, `Team Size`, `School Type`, `% Caucasian`:`% N/A`) %>%
    rename(size = `Team Size`,
           span = `School Type`) %>%
    mutate(Ed_SomeCollege = education[education$Ed_HS == 0,]$ratio * size,
...
initial_placement <- function(seed=42, acm_df, school_df){
  ...
  for (x in acms_with_Manual.Placement$acm_id){
    # Sort by acm_id and resest index so we can make swaps assuming that each index=acm_id
...
```{r}
calculate_score = function(team_placements_df, school_df) {
  
  # Merge  with school_df to pull in school characteristics
  team_placements_df <- merge(team_placements_df, school_df, by.x = "placement", by.y = "sch_id", all.x = TRUE)
  
  #################
  # COMMUTE SCORE #
  #################
  
  # This score is simply the sum number of seconds all ACMs travel to their assigned schools (there might be a better measure)
  ...
  gender_frame <- 
    expand.grid(placement = 1:nrow(school_df), Gender = levels(team_placements_df$Gender))
  ...
  # Nick Question: Why not minimize variance across teams rather than subtracting whole-corps variance from average team variance?
  ...
  # Brainstorming approaches
    ...
    # average of each team's average percent representation of each ethnicity (I think this is what we want)
      # should we count multi-ethnic people as a half-person of each or as a whole person of each?
      # aka would an African American Latino count as 50% of each, or 100% of each as two people.
...
```{r generate logic for invalid placements}
# This is where we will define firm placement rules that cannot be broken (the placement won't be accepted)
...
# initial score
calculate_score(team_placements_df, school_df)$aggr_score

output <- run_intermediate_annealing_process(starting_placements = team_placements_df, school_df = school_df, best_placements = team_placements_df, best_score = 1420788, starting_iteration = 1, number_of_iterations = 1000, s_curve_amplitude = 4000, s_curve_center = 0, s_curve_width = 3000)
...
school_config <- function(school_df, acm_enc){
  ...
  school.data <- select(school_df, `School_ID`, `Team Size`, `School Type`, `% Caucasian`:`% N/A`) %>%
    rename(size = `Team Size`,
           span = `School Type`) %>%
    mutate(HSGrad_tgt = ifelse(span=="ES", 0, education[education$level %in% 'HS',]$ratio * size),
...
team_placements_df <- initial_placement(seed=42, acm_enc, school_df)
team_placements_df
```
...
calculate_score = function(team_placements_df, school_targets) {
  ...
  # Merge  with school_df to pull in school characteristics
  team_placements_df <- merge(team_placements_df, school_targets, by.x = "placement", by.y = "School_ID", all.x = TRUE)
  ...
  gender_frame <- 
    expand.grid(placement = 1:26, Gender = levels(team_placements_df$Gender))
    # Note--need to replace sch_id with the appropriate school id values based off of the data
...
run_intermediate_annealing_process = function(starting_placements, school_df, best_placements, best_score, starting_iteration, number_of_iterations, s_curve_amplitude, s_curve_center, s_curve_width) {
  ...
  # Merge in School characteristics
  best_placements <- merge(best_placements, school_df, by.x = "placement", by.y = "School_ID", all.x = TRUE)
...
```
>>>>>>> 68a154e3aac268e1794f5b0992e52c7f8f55dfe3

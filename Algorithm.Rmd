...
## Chris
  ...
  * Age variance Score
    * remaining challenge: add weights and probably remove some aspects from the loop
  * ethnicity
  * attend CY school - are these all manual placements on a case-by-case basis?
...
generate_fake_df <- function(num_rows, school_df, seed = 42){
  ...
  fake_df <- data.frame(
    ...
    Tutoring.Experience.EL = sample( c('Elementary School', NA), num_rows, replace=TRUE, prob=c(0.5, 1)),
...
write.table(acm_df_fake, file = "Input 1 - ACM Data2.csv", sep=",", row.names=FALSE)
```
...
hs_loss <- function(targets, actuals){
  loss <- ifelse(targets == 0, (targets - actuals) * -1e10, abs(targets - actuals))
  sum(loss)
}
...
calculate_score = function(team_placements_df, school_targets) {
  ...
  # This score is the average of differences between each gender's percent occurrence across the corps and its percent occurrence on each team.
  ...
  team_placements_df$days_old <- as.integer(Sys.Date() - as.Date(as.character(team_placements_df$Birth.Date), format="%m/%d/%Y"))
  ...
  # This score is the average % representation of an ethnicity on a team. For example, 0.44 means that for the average team, the average teammate experiences that his/her personal ethnicity is represented in 44% of the team. We want this number to be low.
  ...
  scores$ethnicity_score <- ethnicity_eths$avg_eths_rep
  ...
  scores$HS_score <- hs_loss(placed$HSGrad_tgt, placed$HS_Grads)
  scores$SomeCol <- sum(abs(placed$SomeCol_tgt - placed$SomeCol))
  scores$Tutoring <- sum(abs(placed$TutExp - placed$Tutoring))
  scores$Spanish <- sum(abs(placed$SpanishNeed - placed$Spanish))
  scores$OtherLang <- sum(abs(placed$OtherLang_tgt - placed$OtherLang))
  ...
  scores$Math <- sum(abs(placed$Math_tgt - placed$MathAble))          

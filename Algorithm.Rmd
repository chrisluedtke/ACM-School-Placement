...
generate_fake_df <- function(num_rows, school_df, seed = 42){
  ...
  fake_df <- data.frame(
    ...
    Full.Name = names <- randomNames(num_rows,
                                     which.names="both",
    ...
    Roommate.Names1 = rep(NA, num_rows),
    
    Roommate.Names2 = rep(NA, num_rows),
    
    Roommate.Names3 = rep(NA, num_rows),
    
    Roommate.Names4 = rep(NA, num_rows),
  ...
  fake_df$Tutoring.Experience.ES[fake_df$Tutoring.Experience == 'No'] <- NA
...
initial_placement <- function(acm_enc, school_targets, prevent_roommates = "No"){
  ...
  for (x in acms_with_Manual.Placement$acm_id){
    ...
    # Choose 1 acm_id to remove from the school at which we want to ensure manual placement is honored
...
qplot(x = c(1:10000), y = current_temperature(c(1:10000), s_curve_amplitude = 3000, s_curve_center = 0, s_curve_width = 2000), geom="line")
...
traceplot(output$trace)
```

Below I am aiming to create a function for the full implementation of the algorithm from loading the data in to final product.
```{r Full Application}

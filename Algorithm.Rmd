...
encode_vars <- function(df){
  ...
  acm_enc <- cbind(acm_enc, dummy(acm_enc$`Tutoring Preference (Math, ELA, or No Pref)`, sep="_tut pref_"))
...
# School dataframe cleanup.  Generally we want a count of the target number of ACMs for the particular variable.  Ethnicity might be the only variable we have to calculate by hand, the rest should be setup that way from the Excel sheet.  All we have to do is narrow down to just the columns we'll be using to calculate the score.

school_clean_up <- function(df){
  school.data <- select(df, -(`School Name`:`School Type`), -(`N Students Total`:`% N/A`))
  school.data
}

acm_enc <- encode_vars(acm_df)
school_targets <- school_clean_up(school_df)
```

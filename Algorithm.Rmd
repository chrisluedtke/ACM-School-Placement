...
## Chris
  * commutes time (DONE!)
  * gender variance (DONE!)
    *  still need to add weights and probably remove some aspects from the loop
  * age (DONE!)
    *  still need to add weights and probably remove some aspects from the loop
  * ethnicity
  * attend CY school
...
calculate_score = function(acm_df, team_placements_df) {
  ...
  acm_df_with_placements$days_old <- Sys.Date() - as.Date(as.character(acm_df_with_placements$Birth.Date), format="%m/%d/%Y")

...
## Chris
  * Done - commutes time
    * remaining challenge: public transit commute calculations don't include time spent waiting for bus/train to arrive
  * Done - gender variance
...
initial_placement <- function(seed=42){
...
team_placements_df <- initial_placement(seed=42)
team_placements_df
```
...
calculate_score = function(acm_df, team_placements_df) {
  ...
  acm_df_with_placements <- merge(acm_df, team_placements_df, by = "acm_id", all.x = TRUE)
  
  # Merge acm_df_with_placements with school_df to pull in school characteristics
  acm_df_with_placements <- merge(acm_df_with_placements, school_df, by.x = "placement", by.y = "sch_id", all.x = TRUE)
  
  #################
  # COMMUTE SCORE #
  #################
  
  # This score is simply the average number of seconds each ACM travels to their assigned school
  ...
  gender_score <- gender_frame_gs$mean_gend_diff
  ...
  acm_df_with_placements$days_old <- Sys.Date() - as.Date(as.character(acm_df_with_placements$Birth.Date), format="%Y-%m-%d")
  
  acm_df_with_placements$days_old <- as.integer(acm_df_with_placements$days_old)
  
  age_var <- 
    group_by(acm_df_with_placements, placement) %>%
    summarize(age_var = var(days_old)) %>%
    ungroup() %>%
    summarize(avg_age_var = mean(age_var))

  age_score <- abs(age_var$avg_age_var - var(acm_df_with_placements$days_old))
...
run_intermediate_annealing_process = function(acm_df, team_placements_df, placement_score, best_placements, best_score, starting_iteration, number_of_iterations, s_curve_amplitude, s_curve_center, s_curve_width) {
  ...
  return(list(avg_commute_secs=candidate_score$commute_score, gender_score=candidate_score$gender_score, age_score=candidate_score$age_score, best_placements=best_placements, best_score=best_score))
}

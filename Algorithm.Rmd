...
## Nick Q's
  * Two different score approaches
    1 - School Targets. Based on ACM survey responses, compute target # ACMs for each category on each team.
      * Benefit: target computation happens outside of the loop
    2 - Stats to compare corps-wide features to current set of team placements
  * How to compare these two scores, how to weight scores in general, what that process looks like
    - run and gun? - run many iterations and eyeball the results, setting weights as needed
  * Better to validate swaps with validation function or to blow up the score of some unfavorable placement

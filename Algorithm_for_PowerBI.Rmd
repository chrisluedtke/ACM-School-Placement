...
# thing <- corps_demographic_targets(school_df, dataset)
...
school_config <- function(school_df, acm_enc){
  ...
  school.data <- select(school_df, `sch_id`, `Team Size`, GradeLevel, `% Hispanic`) %>%
...
calculate_score = function(team_placements_df, school_targets, gender_target=gender_g) {
  ...
  team_placements_filtered <- team_placements_df %>%
  ...
  scores$Edscore <- hs_loss(placed$HSGrad_tgt, placed$HS_Grads) + sum(abs(placed$SomeCol_tgt - placed$SomeCol))
  scores$Tutoring <- sum(abs(placed$TutExp - placed$Tutoring))
  scores$Spanish <- reqd_spanish_loss(placed$SpanishNeed, placed$Spanish, min_reqd = 1) + sum(abs(placed$OtherLang_tgt - placed$OtherLang))
  scores$Math <- sum(abs(placed$Math_tgt - placed$MathAble))
  scores$Gender <- sum(abs(placed$Male_tgt - placed$Gender))
...
run_intermediate_annealing_process = function(starting_placements, school_df, best_placements=starting_placements, best_score=1e12, starting_iteration=1, number_of_iterations) {
  ...
  for(i in 1:number_of_iterations) {
    ...
    temp = current_temperature(iter, 3000, number_of_iterations * .2, number_of_iterations * .1)
...
plot_curve <- function(iters){
  ...
  data <- data.frame(x = 1:iters, y = current_temperature(1:iters, 1000, iters * .1, iters * .1))
...
placement <- function(acm_enc, school_targets, number_iterations) {
  team_placements_df <- initial_placement(acm_enc, school_targets)
    
  output <- run_intermediate_annealing_process(starting_placements = team_placements_df, school_df = school_targets, best_placements = team_placements_df, best_score = 1e12, starting_iteration = 1, number_of_iterations = number_iterations)
}
...
Single_Run <- placement(acm_enc, school_targets, 10000)
...
```{r Efficiency Testing}

iters <- c(100, 1e4, 1e5, 1e6)
time <- c()

...
dataset$number_iterations[1] <- 1000
...
calculate_score = function(team_placements_df, school_targets, gender_target=gender_g) {
  ...
  if(consider_commutes == "Yes"){
    ...
    scores$commute_score <- sqrt( dt_commutes[id_dest %in% team_placements_df$id_dest,  sum(Commute.Time, na.rm = TRUE)] )
  ...
  if(is.na(scores$aggr_score)){
    ...
    scores$error_commute_scores <- NA
  }
...
error_in_scores <- as.data.frame(output$candidate_score)

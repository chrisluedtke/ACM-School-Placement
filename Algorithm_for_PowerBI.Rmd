...
```{r "Power BI Step 2: Clean Roommates and Encode Variables"" echo=FALSE}
library(dplyr)
library(tidyr)
    ...
    for (y in 1:nrow(school_targets)){
      ...
      if (length(dup_roommates) > 0){
        ...
        for (z in  dup_roommates){
          ...
          # if no roommates on team were manually placed, choose 1 of the roommates to keep on the team
          if (length(roommate_set[!is.na(roommate_set$Manual.Placement_id), ]) == 0){
          ...
          for (k in roommate_set_no_manual$acm_id[roommate_set_no_manual$acm_id != acm_to_keep]){
            ...
            school_to_swap_frame <- acms_no_Manual.Placement[acms_no_Manual.Placement$placement == school_to_swap, ]
            
            # choose 1 ACM who has no Manual.Placement, and whose roommates are not on the team we swap them to
            swap2 <- sample(school_to_swap_frame$acm_id[is.na(school_to_swap_frame$Roommate.Names)
                                                        | school_to_swap_frame$Roommate.Names %not in% school_frame$Roommate.Names], 1)

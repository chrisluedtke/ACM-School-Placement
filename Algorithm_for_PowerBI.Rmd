...
# TO DO
# - scores that make sense and can be manually balanced
# - squash ALL the bugs
...
calculate_score = function(team_placements_df, school_targets, gender_target=gender_g) {
  ...
  if(consider_commutes == "Yes"){
    ...
    scores$commute_score <- sqrt( dt_commutes[id_dest %in% team_placements_df$id_dest,  sum(Commute.Time)] )
  ...
  #if(is.na(scores$aggr_score)){
  #  scores$error_commute_scores <- as.data.frame(dt_commutes[id_dest %in% team_placements_df$id_dest])
  #}
...
run_intermediate_annealing_process = function(starting_placements, school_df, best_placements=starting_placements, best_score=1e12, starting_iteration=1, number_of_iterations, center_scale, width_scale) {
  ...
  if(consider_commutes == "Yes"){
    ...
    commutes <- commutes[,c("acm_id", "Commute.Time")]
  ...
  best_placements$acm_id[best_placements$acm_id > nrow(acm_enc)] <- 800:(800 + sum(school_df$size) - nrow(acm_enc))
...
scores_if_error <- as.data.frame(output$candidate_score)

scores <- as.data.frame(output$diff_scores)

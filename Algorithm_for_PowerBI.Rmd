...
prev_roommates <- function(school_targets, team_placements_df){
  ...
  roommie_violations <- 
    team_placements_df[!is.na(team_placements_df$Roommate.Names), ] %>% 
      group_by(placement) %>%
      count(Roommate.Names)
  
  roommie_violations <- roommie_violations[roommie_violations$n > 1, ]
  
  for (z in  roommie_violations$Roommate.Names){
    school1_swap <- roommie_violations$placement[roommie_violations$Roommate.Names == z]
    ...
    for (k in roommate_set_no_manual$acm_id[roommate_set_no_manual$acm_id != acm_to_keep]){
      ...
      # choose school2_swap where no roommate conflict exists with swap1 ((HERE!!! also was searcing "roommate.names !="))
      n_same_roommie <- team_placements_df %>%
        group_by(placement) %>%
        summarise(n = sum(Roommate.Names %in% z))

      school2_swap <- sample(n_same_roommie$placement[n_same_roommie$n == 0], 1)
      
      school2_acms_to_swap <- acms_no_Manual.Placement[acms_no_Manual.Placement$placement == school2_swap, ]
      
      # choose an ACM2 on school2_swap who has no Manual.Placement, and whose roommates are not on school1_swap
      swap2 <- sample(school2_acms_to_swap$acm_id[(is.na(school2_acms_to_swap$Roommate.Names))
                                                  | !(school2_acms_to_swap$Roommate.Names %in% team_placements_df$Roommate.Names[team_placements_df$placement == school1_swap])], 1)
  ...
  for (x in hs_acm_to_swap$acm_id){
    ...
    school1_swap <- hs_acm_to_swap$placement[hs_acm_to_swap$acm_id == x]
    
    acms_to_swap_with <- acms_for_swaps[(acms_for_swaps$Ed_HS == 0) & (acms_for_swaps$span != "High"),]
    
    # If ACM1 from Team1 has roommates, find a non-HS Team2 without those roommates
    if (!is.na(hs_acm_to_swap$Roommate.Names[hs_acm_to_swap$acm_id == x])){
      acm1_roommates <- hs_acm_to_swap$Roommate.Names[hs_acm_to_swap$acm_id == x]
      
      schools_no_roommate_conf <- unique(acms_to_swap_with$placement[(acms_to_swap_with$Roommate.Names != acm1_roommates)
                                                                     | is.na(acms_to_swap_with$Roommate.Names)])
      
      school2_swap <- sample(schools_no_roommate_conf, 1)
      
    # Else choose any non-HS Team2
    } else {
      school2_swap <- sample(unique(acms_for_swaps$placement[acms_for_swaps$span != "High"]), 1)
      
    }
    
    # Find an ACM2 without roomamtes on Team1, and who is eligible to serve in HS
    acms_to_swap_with <- acms_to_swap_with[acms_to_swap_with$placement == school2_swap, ]

    swap2 <- sample(acms_to_swap_with$acm_id[(is.na(acms_to_swap_with$Roommate.Names))
                                             | !(acms_to_swap_with$Roommate.Names %in% team_placements_df$Roommate.Names[team_placements_df$placement == school1_swap])], 1)
...
initial_placement <- function(acm_enc, school_targets, prevent_roommates){
  ...
  if (prevent_roommates == "Yes"){
    prev_roommates(school_targets, team_placements_df)
...
best_placements[best_placements$placement == 6, ]
```
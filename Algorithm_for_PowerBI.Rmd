...
dataset$number_iterations[1] <- 10000
...
# - IJ placements
# - check run times of iterated components

# Score Manual Balance Brainstorming

# How to signify that item doesn't matter at all?
...
if(consider_commutes == "Yes"){
  ...
  dt_commutes$Commute.Time_adj <- dt_commutes$Commute.Time ^ 1.1
}
...
calculate_score = function(team_placements_df, school_targets, gender_target=gender_g) {
  ...
  if(consider_commutes == "Yes"){
    ...
    scores$commute_score <- dt_commutes[id_dest %in% team_placements_df$id_dest, sum(Commute.Time_adj, na.rm = TRUE)] / 200
  ...
  # Used for Isolating Scores
  #scores$commute_score <- 0
  #scores$age_score <- 0
  #scores$ethnicity_score <-0
  #scores$Edscore <- 0
  #scores$Tutoring <-0
  #scores$Spanish <- 0
  #scores$Math <- 0
...
write.table(scores, file = paste(root_dir, "Output - Scores (", gsub(":", ".", Sys.time()), ").csv", sep = ""), sep=",", row.names=FALSE, na = "")

...
Some text that *has some formatting*.
...
eligible_placements_func <- function(team_placements_df, school_df){
  # Things to Check
  # - STATIC - IJ Placement (set IJ PLacement to be ineligilbe as school placement, also not necessary)
  # - STATIC - spanish speaker placements?
  ...
  if(consider_HS_elig == "Yes"){
    perm$HS_conf[(perm$Ed_HS == 1) & (perm$days_old < 365*21) & (perm$GradeLevel == "High")] <- 1
  }
  ...
  perm$pre_mgmt_conf <- as.numeric(mapply(grepl, pattern=perm$`Manager`, perm$Prior.Rship.Name))
  perm$pre_mgmt_conf[is.na(perm$pre_mgmt_conf)] <- 0
  
  # Mark Spanish conflicts where Spanish Speakers are assigned to schools that don't need them
  perm$spanish_conf[(perm$SpanishAble == 1) & (perm$`N Spanish Speakers Reqd` == 0)] <- 1

  # Manual Placements
  perm$MP_conf[!is.na(perm$Manual.Placement) & (perm$School != perm$Manual.Placement)] <- 1

  # Set elig col, overwrite for manual placements
  perm$elig[(perm$HS_conf == 1) | (perm$tl_conf == 1) | (perm$pre_tl_conf == 1) | (perm$pre_mgmt_conf == 1) | (perm$MP_conf == 1) | (perm$spanish_conf == 1)] <- 0
  ...
  perm$acm_id_sch_id <- paste(perm$acm_id, perm$sch_id, sep="_")
 
  # Remove placement column, because that will change each iteration
  elig_placements <- perm[, !names(perm) %in% c("placement")]
  
  return(elig_placements)
}

elig_plcmnt_iter_func <- function(team_placements_df, elig_placements, ij){
  # updates "elig" column with a 0 where roommate conflicts WOULD exist
  ...
  elig_placements_iter <- merge(elig_placements, inter_counts_df, by.x="sch_id", by.y="placement", all.x=TRUE)

  # rm_conf = 1 where there is a roommate conflict
  elig_placements_iter$rm_conf[!is.na(elig_placements_iter$Roommate.Names)] <- as.numeric(mapply(grepl, pattern=elig_placements_iter$Roommate.Names[!is.na(elig_placements_iter$Roommate.Names)], elig_placements_iter$team.roommates[!is.na(elig_placements_iter$Roommate.Names)]))
  
  elig_placements_iter$rm_conf[is.na(elig_placements_iter$rm_conf)] <- 0
  
  # rm_conf = 1 where there is a prev_rel_conf
  elig_placements_iter$prev_rel_conf[!is.na(elig_placements_iter$Prior.Rship.Name1)] <- as.numeric(mapply(grepl, pattern=elig_placements_iter$Prior.Rship.Name1[!is.na(elig_placements_iter$Prior.Rship.Name1)], elig_placements_iter$team.names[!is.na(elig_placements_iter$Prior.Rship.Name1)]))
  
  elig_placements_iter$prev_rel_conf[!is.na(elig_placements_iter$Prior.Rship.Name2) & elig_placements_iter$prev_rel_conf != 1] <- as.numeric(mapply(grepl, pattern=elig_placements_iter$Prior.Rship.Name2[!is.na(elig_placements_iter$Prior.Rship.Name2) & elig_placements_iter$prev_rel_conf != 1], elig_placements_iter$team.names[!is.na(elig_placements_iter$Prior.Rship.Name2) & elig_placements_iter$prev_rel_conf != 1]))
  
  elig_placements_iter$prev_rel_conf[is.na(elig_placements_iter$prev_rel_conf)] <- 0
  
  #if (ij == "IJ teams already exist. Prevent IJ teammates from serving on school teams.") {
  #  elig_placements_iter$ij_conf[!is.na(elig_placements_iter$IJ.Placement)] <- mapply(grepl, pa#ttern=elig_placements_iter$IJ.Placement[!is.na(elig_placements_iter$IJ.Placement)], elig_placements_iter$IJ.Teams[!is.na(elig_placements_iter$IJ.Placement)])
  #  
  #  elig_placements_iter$ij_conf[elig_placements_iter$IJ.Placement == elig_placements_iter$`Team Leader`] <- 1
  #  
  #  elig_placements_iter$ij_conf[is.na(elig_placements_iter$ij_conf)] <- 0
  #  
  #  elig_placements_iter$elig[elig_placements_iter$ij_conf == 1] <- 0
  #}
  
  # Only need to convert to zero rather than 1 because all items in this col were defaulted to 1. 1 means eligible. 0 means not.
  elig_placements_iter$elig[elig_placements_iter$prev_rel_conf == 1 & is.na(elig_placements_iter$Manual.Placement)] <- 0
  elig_placements_iter$elig[elig_placements_iter$rm_conf == 1 & is.na(elig_placements_iter$Manual.Placement)] <- 0

  return(elig_placements_iter)
}
...
initial_valid_placement <- function(team_placements_df, school_df, prevent_roommates, ij){
  ...
  elig_placements <- eligible_placements_func(team_placements_df, school_df)
  ...
  for(x in 1:3000){
    ...
    if(prevent_roommates == "Yes"){
      elig_placements_iter <- elig_plcmnt_iter_func(team_placements_df, elig_placements, ij)
      ...
      elig_placements_iter$elig <- (elig_placements_iter$HS_conf + 
                                      ...
                                      elig_placements_iter$pre_mgmt_conf +
...
run_intermediate_annealing_process = function(starting_placements, school_df, best_placements=starting_placements, number_of_iterations, center_scale, width_scale) {
  ...
  #Pre-generate eligibility of all ACMs to all schools
  elig_placements <- eligible_placements_func(team_placements_df, school_df)
  ...
  for(i in 1:number_of_iterations) {
    ...
    if(prevent_roommates == "Yes"){
      elig_placements_iter <- elig_plcmnt_iter_func(candidate_placements_df, elig_placements, ij)
    } else {
      elig_placements_iter <- elig_placements
    }
    ...
    swap2_set <- elig_placements_iter$acm_id[elig_placements_iter$elig==1 & 
                                               elig_placements_iter$sch_id == schools_to_swap[1] & 
                                               elig_placements_iter$acm_id %in% candidate_placements_df$acm_id[candidate_placements_df$placement == schools_to_swap[2]]]
...
```{r}
# Full Application
acm_df <- read.csv(file = paste(root_dir, "Input 1 - ACM Data - IJ.csv", sep = ""), check.names=FALSE, stringsAsFactors = FALSE)
...
school_df <- read_excel(path = paste(root_dir, "Input 2 - School Data - IJ.xlsx", sep = ""))

...
### TODO: Survey changes
# * currently enters infinite loop in initial_valid_placement() when inputting dummy ACM dataframce
...
```{r Replicate Parameters from Power BI, eval=FALSE}
...
# If package failed to load above, and it will print the error here.
...
user_path <- dirname(path.expand('~'))
user_path <- paste0(user_path, '/ACM-School-Placement/')

acm_input_path <- paste0(user_path, dataset$acm_input_path)
sch_input_path <- paste0(user_path, dataset$sch_input_path)
output_path <- paste0(user_path, 'outputs/')
acm_commute_path <- paste0(output_path, 'FY17 CHI ACM Commutes.csv') #TODO
...
rename_headers <- function(acm_df){
  vars_df <- read_excel(path = paste0(user_path, "Survey Items to Variable Names.xls"))
...
initial_valid_placement <- function(team_placements_df, school_df, elig_plc_schwise_df, elig_plc_acmwise_df){
  ...
  while (n_inelig > 0){
    i <- i + 1
    if (i > 100){stop('initial_valid_placement() could not find valid starting placements in 100 attempts')}
...
calculate_score <- function(team_placements_df, school_targets, gender_target=gender_g, score_weights=score_factors) {
...
# TODO: scale factors by user importance setting

acm_df <- read.csv(file = acm_input_path, check.names=FALSE, stringsAsFactors=FALSE)
...
school_df <- read_excel(path = sch_input_path)
...
# TODO: fails when blank Manual.Placement col: "Error in left_join_impl(x, y, by$x, by$y, suffix$x, suffix$y) : Can't join on 'School' x 'Manual.Placement' because of incompatible types (character / logical)"

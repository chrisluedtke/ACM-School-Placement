...
```{r PowerBI_Parameters, eval=TRUE}
...
#dataset$project_path <- 'C:/Users/City_Year/GitHub/ACM-School-Placement'
...
dataset$commute_factor <- 0 # Yes/No in PowerBI, converted to 1/0
dataset$age_factor <- 0 # hard-coded as 0 below
dataset$ethnicity_factor <- 0
dataset$gender_factor <- 0
dataset$Spanish_factor <- 0 # hard-coded as 0 below, remove when firm placements done
dataset$Tutoring_factor <- 0 # hard-coded as 0 below
dataset$Edscore_factor <- 0
dataset$Math_factor <- 0 # hard-coded as 0 below, remove?
```

```{r Set Working Directory, eval=FALSE}
# We set absolute directories to allow certainty of processin Power BI implementation. Can set eval=TRUE or eval=FALSE when running in R. eval=FALSE in webapp.
...
acm_commute_path <- paste0(output_path, 'Commutes.csv') #TODO update this path to the commute script output

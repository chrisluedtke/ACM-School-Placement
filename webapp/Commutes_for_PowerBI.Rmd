...
dataset$arrival_date <- "2018-05-15"
...
arrival_date <- dataset$arrival_date
```

```{r Define_Functions}
shape_inputs = function(acm_df, school_df){
  if(used_surveygizmo == TRUE){
...
if(dataset$calc_commutes == TRUE){
  ...
  acm_commutes <- commute_procedure(acm_df_clean[2,], school_df_clean)
  end.time <- Sys.time()
  # Write to .csv
  write.table(acm_commutes, file = paste0(output_path, "Output_Commutes.csv"), sep=",", row.names=FALSE, na = "")
